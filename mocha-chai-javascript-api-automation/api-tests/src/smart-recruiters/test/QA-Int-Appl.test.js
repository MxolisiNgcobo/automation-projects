...
const { data_msg_ussdstr,
    ...
    it('POST /Are you an Anglo American Employee = Yes', () => {
        return baseUrl
        .post('/ussd/dialer')
        .set('Cookie', `${Cookie}`)
        .send(data_msg_1)
        .then((res) => {
            expect(res).to.not.be.empty;
            expect(res.statusCode).to.eq(200);
            expect(res.body.msg).to.contain("Please provide Employee ID");
        }); 
    });

    it('POST /Employee ID (Valid)', () => {
        return baseUrl
        .post('/ussd/dialer')
        .set('Cookie', `${Cookie}`)
        .send(data_msg_empno)
        .then((res) => {
            expect(res).to.not.be.empty;
            expect(res.statusCode).to.eq(200);
            expect(res.body.msg).to.contain("Please provide Date of birth");
        }); 
    });

    it('POST /Please provide Date of birth (Valid)', () => {
        return baseUrl
        .post('/ussd/dialer')
        .set('Cookie', `${Cookie}`)
        .send(data_msg_dob)
    ...
    it('POST /Do you have the right to work', () => {
        ...
        .then((res) => {
            expect(res).to.not.be.empty;
            expect(res.statusCode).to.eq(200);
            expect(res.body.msg).to.contain("What is your employment status");
        }); 
    });

    it('POST /What is your employment status', () => {
        return baseUrl
        .post('/ussd/dialer')
        .set('Cookie', `${Cookie}`)
        .send(data_msg_3)
    ...
    it('POST /Insert your 13 digit ID (Valid ID)', () => {
        return baseUrl
        .post('/ussd/dialer')
        .set('Cookie', `${Cookie}`)
